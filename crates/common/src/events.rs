...
//!
//! The event type is the seam's own; what the queue needs of it is [`Queued`].
...
/// What the queue asks of an event.
pub trait Queued {
    /// The bytes an event carries beyond its fixed fields, which is what the
    /// byte budget counts and what a take into the caller's buffer copies.
    ...
    /// saying why everything stopped leaves an application with a session that
    /// went quiet and nothing to report; every other event is one of a stream
    /// of them, and losing the oldest is what the dropped count is for.
...
impl<E: Queued> State<E> {
    fn push(&mut self, event: E) {
        ...
        while self.queued.iter().any(|queued| !queued.undroppable())
            && (self.queued.len() >= MAX_EVENTS || self.bytes + arriving > MAX_BYTES)
        {
            ...
            // by a count.
            let Some(at) = self.queued.iter().position(|queued| !queued.undroppable()) else {
                break;
            };
    ...
    fn take(&mut self) -> Option<Received<E>> {
...
impl<E> Shared<E> {
    ...
    fn state(&self) -> MutexGuard<'_, State<E>> {
...
impl<E: Queued> Receiver<E> {
    /// Take one if there is one. Never blocks.
    pub fn try_recv(&self) -> Option<Received<E>> {
        self.shared.state().take()
    }

    /// Take one, waiting up to `timeout` for it to arrive.
    pub fn recv_timeout(&self, timeout: Duration) -> Option<Received<E>> {
    ...
    fn wait_for(
        &self,
        timeout: Duration,
        mut take: impl FnMut(&mut State<E>) -> Option<Delivery<E>>,
    ) -> Delivery<E> {
        ...
        loop {
            ...
            crate::wait::wait(&self.shared.arrivals, seen, left);
        }
    }
}
...
pub fn queue<E: Queued>() -> (Sender<E>, Receiver<E>) {
...
mod tests {
    ...
    /// The shape a seam's event set has: a stream of small ones, one that
    /// carries a body, and one that must never be dropped.
    #[derive(Debug, Clone, PartialEq, Eq)]
    pub(super) enum Event {
        Ready { attempt: String },
        UserData { text: Vec<u8> },
        Fatal { reason: i32 },
    }
    ...
    fn an_empty_queue_waits_for_the_timeout_and_then_gives_up() {
        let (_sender, receiver) = queue::<Event>();
        let began = clock::Time::now();
...
mod delivery_tests {
    ...
    fn a_body_is_copied_into_the_callers_buffer() {
        ...
        assert!(matches!(taken.event, Event::UserData { .. }));

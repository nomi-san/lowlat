...
use super::{Acquired, Capturable, Selectable};
...
impl Display {
    ...
    ///
    /// **One per call, consumed by [`Display::wait_present`].** A refused arm
    ...
    /// Wait up to `timeout_ms` for the present an arm asked about.
    ///
    /// Nothing when the display said nothing in the time. Otherwise whether
    /// a present was among what it said: the events accumulate while the loop
    /// is busy, so they are drained rather than read one at a time, and a
    /// failed wait reports no present.
    pub fn wait_present(&self, timeout_ms: i32) -> Option<bool> {
        let mut pollfd = libc::pollfd {
            fd: std::os::fd::AsRawFd::as_raw_fd(&self.card.as_fd()),
            events: libc::POLLIN,
            revents: 0,
        };
        // SAFETY: the descriptor is the display's own, which outlives the
        // call, and the array is a live local.
        let ready = unsafe { libc::poll(&raw mut pollfd, 1, timeout_ms) };
        match ready {
            0 => None,
            ready if ready > 0 => Some(self.card.drain_events().unwrap_or(false)),
            _ => Some(false),
        }
    }

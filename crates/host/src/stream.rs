...
/// What a stream is built on -- the display's device, the encoder that takes
/// its frames, the census of what those could code -- which is the
/// platform's; the loop in this module is written once.
#[cfg(target_os = "linux")]
#[path = "stream/linux.rs"]
mod sys;
...
fn vblank_event(desktop: &crate::display::Display, budget_ms: f64) -> bool {
    ...
    while lowlat_common::clock::diff_ms(began, lowlat_common::clock::Time::now()) < budget_ms {
        ...
        if let Some(presented) = desktop.wait_present(timeout) {
            return presented;
        }
    }
    false
...
impl Shared {
    ...
    fn publish_place(&self, place: Option<lowlat_capture::Placement>) {
...
fn run(
    ...
    loop {
        ...
        let exit = sys::build(shared, arrivals, &config, &mut roster);
        match exit {
            Exit::Stopped => return,
    ...
    loop {
        ...
        if asked & lowlat_core::init::FLAG_COLOR444 != 0 {
            if full_chroma_census.is_none() {
                let gate = sys::chroma_census(&config);
                if let Err(why) = gate {
...
mod tests {
    ...
    fn full_chroma_is_refused_when_the_third_encoder_is_preferred() {
        ...
        let Err(gate) = sys::chroma_census(&config) else {
    ...
    fn a_mixed_machine_refuses_full_chroma_with_the_gate_named() {
        if let Err(gate) = sys::chroma_census(&test_config(Codec::H265)) {
            assert!(

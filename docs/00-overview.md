...
| D12 | **v1 emulates one virtual controller, the Xbox 360 pad, on the kernel input layer.** Linux offers two layers and the controller being emulated picks one rather than preference deciding: the 360 pad speaks a vendor protocol on a vendor-class interface and its driver binds on the USB bus, so the HID layer cannot present it at all, while a DualShock 4 or DualSense is worth emulating only through the HID layer, because the point of choosing one is that applications recognise it as one and that recognition comes from the kernel's own driver claiming the report descriptor ([07 §4.2](07-platforms.md)). A peer sends the same button layout whichever controller it is holding, so a second emulation buys identity and not capability, and that identity costs a device-setup handshake the kernel driver will refuse to complete against a device that cannot answer it. Deferred, not dropped. The host-side mode-versus-type distinction the established SDK exposes has no meaning here and is not offered. |

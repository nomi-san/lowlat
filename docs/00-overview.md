...
| D7 | **v1 codecs are H.264 and HEVC. 8-bit 4:2:0 is the default; 10-bit is v1 on HEVC and 4:4:4 is out.** Encoder backends are **VAAPI and NVENC together at v1**, plus Vulkan Video as an explicit third choice; amended 2026-08-15 after the platform probe measured working hardware encode on the open stack, and again 2026-08-30 once colour was measured rather than assumed. **10-bit is in** because the compositor scans out a 10-bit framebuffer for the ordinary desktop, so the loss is ours at the write and nowhere else; every part that can host here encodes and decodes HEVC Main10, and the depth is settled per session by what the guests declare rather than by a host setting. **4:4:4 is out, and the measurement is why it is not merely deferred**: it costs 0.22 ms a picture and 1.39x the bytes at eight bits and 1.86x the bytes at ten on identical content, and one of the three encoders cannot produce it at any bit depth, so under D11 a host whose captured output moved to that device would have to end the session rather than downgrade. The remaining unknowns are settled and sized in Phase 11.6: the open stack wants one packed surface where the vendor interface wants three planar ones, so the conversion is one body per layout rather than a depth uniform. Both wire flag bits already exist; only the depth one is ever set. |

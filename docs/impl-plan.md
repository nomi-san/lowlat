...
1. [x] **Decoded pixels match a ten-bit source on every backend that claims the depth**, with a
   ...
   *Met on all three, 2026-09-22.* The open stack is measured over sixty pictures against the
   ...
   *The vendor backend measured 2026-09-22*: its ten-bit test now runs past its in-flight depth
   ...
   Vulkan backend measured the same day*, through the path the host drives -- the conversion
   writing the encoder's own picture, which nothing reads back -- by converting every source
   picture a second time into a target the conversion device can read and keeping that beside
   the stream, since the same shader on the same input writes the same samples: sixty
   pictures of a coloured bar moving over a coloured field at 1920x1080, whose bottom rows
   are the partial block that once went wrong, decoded as Main 10 -- 60 of 60 -- read 70.5 dB
   in luma on the refresh picture and bit for bit on every predicted one, chroma no worse
   than 54 dB, flat. The vendor's device on the same interface lends no writable picture and
   is passed over there as the host passes it over. The source is an eight-bit picture

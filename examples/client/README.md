...
Three chords are the demo's and are never sent: `Ctrl+Alt+F` switches the
picture between stretched to the window and shown at its own size,
`Ctrl+Alt+R` lets go of a captured pointer (and takes it again),
`Ctrl+Alt+O` asks the host to stream its next output, through the same
application protocol a settings panel uses. A bare Windows key is not sent,
because the desktop here takes it and the host would be left holding the
modifier; it reaches the host on chords.

The key table `keys.h` is generated by `scripts/gen-client-keys.py` from the
toolkit's own Linux key map and the kernel's usage table.

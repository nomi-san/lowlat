...
// An extension entry point: null when the context lacks it, and the feature
// that needs it says so rather than the loader failing.
#define GLPROC_LOAD_OPTIONAL(name) \
	if (!name) \
		name = MTY_GLGetProcAddress(#name);
...
static bool glproc_global_init(void)
{
	...
	if (!GLPROC_INIT) {
		...
		GLPROC_LOAD_OPTIONAL(glCreateMemoryObjectsEXT);
		GLPROC_LOAD_OPTIONAL(glDeleteMemoryObjectsEXT);
		GLPROC_LOAD_OPTIONAL(glImportMemoryFdEXT);
		GLPROC_LOAD_OPTIONAL(glBufferStorageMemEXT);
